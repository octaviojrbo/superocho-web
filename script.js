...
async function loadHomePromotion(){
  ...
  const { data, error } = await supabaseClient
    .from("promotions")
    .select("image_url, display_order, created_at")
  ...
  if(error){
    console.error("Error cargando la promoción de inicio:", error);
    container.innerHTML = "";
    return;
  }
  ...
  container.innerHTML = `
    <div class="home-promo-image-card">
      <img src="${promo.image_url}" alt="Promoción Super 8">
    </div>
  `;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (menuButton && menu) {
    ...
    menuButton.addEventListener("click", () => window.setTimeout(syncMenuState, 0));
    menu.querySelectorAll("a").forEach(link => link.addEventListener("click", syncMenuState));
  ...
  document.querySelectorAll(".topbar a, .mobile-menu a").forEach(link => {
...
// Mantiene la sección de sucursales preparada, pero todavía fuera del sitio público.
document.addEventListener('click', (event) => {
  const link = event.target.closest('.coming-soon-link');
  if (!link) return;
